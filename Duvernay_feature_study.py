...
train1 = training_df.drop(['UWI','Set','True1(C)','True2(C)','True_TD(C)','Static_Temp (C)','TrueTemp'], axis=1)
...
reg_xgr = xgr.fit(train1, training_df['True_TD(C)'].values.reshape(-1, 1))
n_features = train1.shape[1]
plt.figure(figsize=(10,8))
plt.barh(range(n_features), xgr.feature_importances_, align='center')
plt.yticks(np.arange(n_features), list(train1))
...
master_df.corr().to_csv('D4D/Duvernay/AB_master_df_corr.csv')
print("\nCorrelation (Training): ")
print(training_df.corr())
training_df.corr().to_csv('D4D/Duvernay/AB_training_df_corr.csv')